...
export class DexTokens {
  ...
  addTokenInfos(tokenInfos: TokenInfo[]): DexTokens {
    const notExists = tokenInfos.filter(
      (a) => !this.tokenInfos.some((b) => a.id === b.id)
    );
  ...
  addTokenPairs(tokenPairs: TokenPair[]): DexTokens {
    const notExists = tokenPairs.filter(
      (a) => !this.tokenPairs.some((b) => a.tokenPairId === b.tokenPairId)
    );
  ...
  getAllowedTokenInfos(tokenId: string | undefined): TokenInfo[] {
    ...
    return this.tokenInfos.filter((tokenInfo) =>
      allowed.includes(tokenInfo.id)
    );
  }
}
...
export async function getTokenPairState(
  tokenAInfo: TokenInfo,
  tokenBInfo: TokenInfo
): Promise<TokenPairState> {
...
function _getAmountIn(
  ...
  if (amountOut >= reserveOut) {
    throw new Error(
      `amout must less than reserve, amount: ${bigIntToString(
        amountOut,
        tokenOutInfo.decimals
      )}, reserve: ${bigIntToString(reserveOut, tokenOutInfo.decimals)}`
    );
  }
...
export function getAmountOut(
  ...
  if (tokenInId === state.token0Info.id)
    return _getAmountOut(amountIn, state.reserve0, state.reserve1);
...
function calcPriceImpact(
  ...
  const [reserveIn, reserveOut] =
    state.token0Info.id === tokenInId
      ? [state.reserve0, state.reserve1]
      : [state.reserve1, state.reserve0];
  const numerator =
    (reserveOut * (reserveIn + amountIn) -
      (reserveOut - amountOut) * reserveIn) *
    100n;
  const denumerator = reserveIn * (reserveOut - amountOut);
  const impact = BigNumber(numerator.toString())
    .div(BigNumber(denumerator.toString()))
    .toFixed();
...
export function getAddLiquidityDetails(
  state: TokenPairState,
  inputTokenId: string,
  inputAmount: bigint,
  ...
  const [reserveA, reserveB] =
    inputTokenId === state.token0Info.id
      ? [state.reserve0, state.reserve1]
      : [state.reserve1, state.reserve0];
...
export async function addLiquidity(
  ...
  const isInitial =
    tokenPairState.reserve0 === 0n && tokenPairState.reserve1 === 0n;
  const amountAMin = isInitial
    ? amountADesired
    : minimalAmount(amountADesired, slippage);
  const amountBMin = isInitial
    ? amountBDesired
    : minimalAmount(amountBDesired, slippage);
  const [amount0Desired, amount1Desired, amount0Min, amount1Min] =
    tokenAInfo.id === tokenPairState.token0Info.id
...
export function getRemoveLiquidityDetails(
  ...
  const amount0 =
    (liquidity * tokenPairState.reserve0) / tokenPairState.totalSupply;
  const amount1 =
    (liquidity * tokenPairState.reserve1) / tokenPairState.totalSupply;
...
export async function removeLiquidity(
  ...
  const result = await RemoveLiquidity.execute(signer, {
    ...
    attoAlphAmount:
      extraDustAmount(state.token0Info.id, state.token1Info.id) + DUST_AMOUNT, // `DUST_AMOUNT` for liquidity token
...
export async function tokenPairExist(
...
export function tryStringToBigInt(
...
export function tryBigIntToString(
...
export function tryGetBalance(
  ...
  return balance === undefined
    ? '0'
    : bigIntToString(balance, tokenInfo.decimals);
}

export function tokenPairMatch(
  tokenPairState: TokenPairState | undefined,
  token0Info: TokenInfo | undefined,
  token1Info: TokenInfo | undefined
) {
  return (
    (tokenPairState?.token0Info.id === token0Info?.id &&
      tokenPairState?.token1Info.id === token1Info?.id) ||
