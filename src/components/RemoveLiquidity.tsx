...
function RemoveLiquidity() {
  ...
  const [totalLiquidityAmount, setTotalLiquidityAmount] = useState<
    bigint | undefined
  >(undefined);
  const [removeLiquidityDetails, setRemoveLiquidityDetails] = useState<
  ...
  const { tokenPairState, getTokenPairStateError } = useTokenPairState(
    tokenAInfo,
    tokenBInfo
  );
  ...
  useEffect(() => {
    ...
    try {
      if (
        ...
        const result = getRemoveLiquidityDetails(
          tokenPairState,
  ...
  const handleAmountChanged = useCallback((event) => {
    setError(undefined);
    setRemoveLiquidityDetails(undefined);
  ...
  const removeLiquidityButton = (
    <ButtonWithLoader
      ...
      className={
        ...
        (!readyToRemoveLiquidity ? ' ' + classes.disabled : '')
      }
    >
      Remove Liquidity
    </ButtonWithLoader>
  ...
  return (
    <Container className={classes.centeredContainer} maxWidth="sm">
      ...
      <Paper className={classes.mainPaper}>
        ...
        <div>
          <Collapse
            in={!removingLiquidity && !completed && wallet !== undefined}
          >
            {
              <>
                {tokenPairContent}
...
function RemoveLiquidityDetailsCard({
  details,
  amount,
}: {
  details: RemoveLiquidityDetails | undefined;
  ...
  const getTokenAmount = (tokenInfo: TokenInfo): string => {
    const tokenAmount =
      tokenInfo.id === details.token0Id ? details.amount0 : details.amount1;
    ...
    >
      ...
      >
        <DetailItem
          ...
          itemValue={`${getTokenAmount(state.token0Info)} ${
            state.token0Info.symbol
          }`}
        />
        <DetailItem
          itemName={`The number of ${state.token1Info.symbol} you will receive:`}
          itemValue={`${getTokenAmount(state.token1Info)} ${
            state.token1Info.symbol
          }`}
        />
        <DetailItem
