...
function calcPrice(
  ...
  const price = BigNumber(numerator.toString()).div(
    BigNumber(denumerator.toString())
  );
...
function SwapDetailsCard({
  ...
  const [[reserveIn, tokenInDecimals], [reserveOut, tokenOutDecimals]] =
    state.token0Info.id === tokenInInfo.id
      ? [
          [state.reserve0, state.token0Info.decimals],
          [state.reserve1, state.token1Info.decimals],
        ]
      : [
          [state.reserve1, state.token1Info.decimals],
          [state.reserve0, state.token0Info.decimals],
  ...
  return (
    ...
    >
      ...
      >
        <DetailItem
          itemName="Price:"
          itemValue={`1 ${tokenOutInfo.symbol} ${calcPrice(
            reserveIn,
            tokenInDecimals,
            reserveOut,
            tokenOutDecimals
        ...
        <DetailItem
          itemName={
            swapType === 'ExactIn'
              ? 'Minimal received after slippage:'
