...
export default function TokenSelectDialog({
  ...
  const availableTokens = TokenList.filter(
    (tokenInfo) => tokenInfo.id !== tokenId && tokenInfo.id !== counterpart
  ).map((token) => (
    <TokenOptions
      key={token.id}
      tokenInfo={token}
  ...
  return (
    <>
      <Card onClick={handleClick} raised className={style}>
        {info ? (
          <>
            <img src={info.logoURI} className={classes.icon} alt={info.name} />
      ...
      <MyDialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle>
        ...
        <List className={classes.tokenList}>{availableTokens}</List>
      </MyDialog>
    </>
  );
}
